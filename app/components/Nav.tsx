  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${
        scrolled
          ? "bg-ink-rich/95 backdrop-blur-md border-b border-white/10 shadow-lg py-2"
          : "bg-ink-rich py-2"
      }`}
    >
      <nav className="mx-auto flex max-w-6xl items-center justify-between px-6 lg:px-12">
        ...
        <ul className="hidden items-center gap-8 md:flex">
          ...
          <li>
            <a
              href="#contato"
              // Adicionado "gap-2" para espaçar o ícone do texto
        ...
        <div className="absolute top-full left-0 w-full border-t border-white/10 bg-ink-rich md:hidden shadow-xl">
          <ul className="flex flex-col px-6 py-6 gap-2">
            ...
            <li className="pt-4">
              <a
                href="#contato"
                // Adicionado "gap-2" no botão mobile
                className="flex h-12 items-center justify-center gap-2 rounded-sm bg-accent text-sm font-bold uppercase tracking-widest text-white transition-colors hover:bg-accent-dim focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"
                onClick={() => setOpen(false)}
              >
                {/* Ícone do WhatsApp injetado (versão levemente maior para mobile) */}
